...
class AlbumUser(HttpUser):
    ...
    def on_start(self) -> None:
        ...
        self.username = f"locust-user-{RUN_TOKEN}-{user_id}"
    ...
    def _open_archive(self) -> tuple[str, dict[str, Any]] | None:
    ...
    def _upload_photo(self, csrf_token: str, upload_form: dict[str, Any]) -> int | None:
        photo_id = next(PHOTO_COUNTER)
        photo_name = f"locust-photo-{RUN_TOKEN}-{photo_id}"
    ...
    def _open_preview(self, photo_id: int) -> str | None:
    ...
    def _fetch_image(self, photo_id: int) -> bool:
        with self.client.get(
    ...
    def _delete_photo(self, photo_id: int, csrf_token: str) -> bool:
        payload = {
            "csrfmiddlewaretoken": csrf_token,
            "sort": "date",
            "order": "desc",
